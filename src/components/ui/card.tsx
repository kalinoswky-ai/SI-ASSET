...
const Card = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, style, ...props }, ref) => (
    <div
      ref={ref}
      className={cn('eams-card rounded-2xl border backdrop-blur-xl transition-colors duration-300', className)}
      style={{ background: 'var(--surface)', borderColor: 'var(--surface-border)', color: 'var(--text-primary)', ...style }}
      {...props}
    />
  )
)
